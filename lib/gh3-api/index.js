...
GitHubApiV3.prototype.getCommentsUrlsFromDiff = function(diff) {
    var commits = diff.commits.filter(function filterEmptyComments(commit) {
        return commit.commit.comment_count > 0;
    });
    var comments = commits.map(function getCommentUrl(comment) {
        return comment.comments_url;
    });
    comments.forEach(function getCommentBody(comment_url) {
